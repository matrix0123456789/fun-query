...
use MKrawczyk\FunQuery\FQ;
use MKrawczyk\FunQuery\FunQuery;
use PHPUnit\Framework\TestCase;

class PipelineNodeStaticTest extends TestCase
{
...
//        $wanted = ['cat', 'dog'];
//
//        $fourLegsAlphabetically = FQ::filter(fn($x) => $x->legs === 4, $data)->sort(fn($x) => $x->kind)->map(fn($x) => $x->kind);
...
//        $obj = FQ::filter(fn() => true, $original);
...
//    {
//        $original = [2, 4, 6, 8];
//        $result = FQ::some(fn($x) => $x == 4,$original);
...
//        $input = [];
//        $input[] = $this->generator()|>FQ::limit(3,...);
...
//    {
//        $input = ['a', 'b', 'v'];
//        $result = $input|>FQ::count(...);
//        $this->assertEquals(3, $result);
//    }
//
//    public function testCountGenerator()
//    {
//        $result = $this->generator()|>FQ::limit(20,...)|>FQ::count(...);
